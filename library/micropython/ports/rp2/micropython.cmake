# When included from a parent CMakeLists.txt as a library, set MICROPY_AS_LIBRARY to ON
# before including this file. Then this file creates a static library (default name
# micropython_lib) instead of an executable and does not call project() or pico_sdk_init().
if(NOT DEFINED MICROPY_AS_LIBRARY)
    set(MICROPY_AS_LIBRARY OFF)
endif()

# Set main target and component locations
if(MICROPY_AS_LIBRARY)
    if(NOT MICROPY_LIBRARY_NAME)
        set(MICROPY_LIBRARY_NAME micropython_lib)
    endif()
    set(MICROPY_TARGET ${MICROPY_LIBRARY_NAME})
else()
    # Set build type to reduce firmware size (standalone build only)
...
elseif(NOT PICO_SDK_PATH)
    # In library mode the parent usually sets PICO_SDK_PATH via pico_sdk_import.cmake
...
if(NOT MICROPY_AS_LIBRARY)
    include(${PICO_SDK_PATH}/pico_sdk_init.cmake)

    # Define the top-level project (standalone build only)
...
set(MICROPY_SOURCE_PORT
    ${MICROPY_PORT_DIR}/clocks_extra.c
    ${MICROPY_PORT_DIR}/datetime_patch.c
    ...
    $<$<NOT:$<BOOL:${MICROPY_AS_LIBRARY}>>:${MICROPY_PORT_DIR}/main.c>
    ${MICROPY_PORT_DIR}/modrp2.c
    #${MICROPY_PORT_DIR}/micropymain.c
    ${MICROPY_PORT_DIR}/mphalport.c
    ${MICROPY_PORT_DIR}/mpnetworkport.c
    ${MICROPY_PORT_DIR}/mpthreadport.c
    ${MICROPY_PORT_DIR}/mutex_extra.c
    ${MICROPY_PORT_DIR}/pendsv.c
    ${MICROPY_PORT_DIR}/rp2_flash.c
    ${MICROPY_PORT_DIR}/rp2_pio.c
    ${MICROPY_PORT_DIR}/rp2_psram.c
    ${MICROPY_PORT_DIR}/rp2_dma.c
    ${MICROPY_PORT_DIR}/uart.c
    ${MICROPY_PORT_DIR}/usbd.c
    ${MICROPY_PORT_DIR}/msc_disk.c
    ${MICROPY_PORT_DIR}/mbedtls/mbedtls_port.c
...
if (MICROPY_PY_LWIP)
    target_link_libraries(${MICROPY_TARGET} PRIVATE micropy_lib_lwip)
...
if(MICROPY_PY_BLUETOOTH)
    list(APPEND MICROPY_SOURCE_PORT ${MICROPY_PORT_DIR}/mpbthciport.c)
    target_compile_definitions(${MICROPY_TARGET} PRIVATE
...
if (MICROPY_BLUETOOTH_BTSTACK)
    ...
    list(APPEND MICROPY_SOURCE_PORT ${MICROPY_PORT_DIR}/mpbtstackport.c)

    include(${MICROPY_DIR}/extmod/btstack/btstack.cmake)
    target_link_libraries(${MICROPY_TARGET} PRIVATE micropy_extmod_btstack)
...
if(MICROPY_BLUETOOTH_NIMBLE)
    ...
    list(APPEND MICROPY_SOURCE_PORT ${MICROPY_PORT_DIR}/mpnimbleport.c)
    target_compile_definitions(${MICROPY_TARGET} PRIVATE
    ...
    target_link_libraries(${MICROPY_TARGET} PRIVATE micropy_extmod_nimble)
...
if (MICROPY_PY_NETWORK_CYW43)
    ...
    list(APPEND MICROPY_SOURCE_PORT
        ${MICROPY_PORT_DIR}/machine_pin_cyw43.c
    )
...
target_link_libraries(${MICROPY_TARGET} PRIVATE micropy_lib_mbedtls)

target_link_libraries(${MICROPY_TARGET} PRIVATE usermod)

if(MICROPY_AS_LIBRARY)
    set(_MICROPY_INC_VISIBILITY PUBLIC)
else()
    set(_MICROPY_INC_VISIBILITY PRIVATE)
endif()

target_include_directories(${MICROPY_TARGET} ${_MICROPY_INC_VISIBILITY}
    ${MICROPY_INC_CORE}
    ${MICROPY_INC_USERMOD}
    ${MICROPY_BOARD_DIR}
    "${MICROPY_PORT_DIR}"
...
# When building as a library, use INTERFACE so link options propagate to the executable that links us
if(MICROPY_AS_LIBRARY)
    set(MICROPY_LINK_VISIBILITY INTERFACE)
else()
    set(MICROPY_LINK_VISIBILITY PRIVATE)
endif()

target_link_options(${MICROPY_TARGET} ${MICROPY_LINK_VISIBILITY}
...
# Linker script and executable-only outputs (standalone build only)
if(NOT MICROPY_AS_LIBRARY)
