...
extern void picoterm_readline(char *buffer, size_t size);
uint8_t columns = 40;
...
static const command_t commands[] = {
    ...
    {"ls", ls, "List files on the SD card"},
    ...
    {"python", python, "start python repl"},
...
void run_command(const char *command)
{
    ...
    for (int i = 0; commands[i].name != NULL; i++)
    {
        if (strcmp(cmd_args[0], commands[i].name) == 0)
        {
            ...
            else if (strcmp(cmd_args[0], "ls") == 0 && cmd_args[1] != NULL)
            {
...
void sd_mkfile_filename(const char *filename)
{
    ...
    {
        printf("> ");
        picoterm_readline(line, sizeof(line));
        if (strcmp(line, ".") == 0)
        {
